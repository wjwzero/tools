...
import com.tools.common.excel.model.*;
import com.tools.common.excel.util.ReflectUtils;
...
public class DataAssembleHandle implements ExcelHandle {
    ...
    private void mergeRow(RowCol rowCol, ExcelHeader[][] headers) {
        ...
        for (int i = 0; i < row; i++) {
            ...
            count = new HashMap<String, Integer>(10);
            // 计算每个名称出现次数
            for (int j = 0; j < col; j++) {
    ...
    private void setAvailableHeaders(RowCol rowCol, ExcelHeader[][] headers) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                if (!isHeaderEmpty(headers[i][j])) {
                    int preIndex = j - 1;
                    if (preIndex >= 0 && !isHeaderEmpty(headers[i][preIndex]) && !isHeaderEmpty(headers[i][j])) {
    ...
    private boolean isHeaderEmpty(ExcelHeader excelHeader) {
        return null == excelHeader || StringUtils.isEmpty(excelHeader.getName());
    }
    ...
    private void assembleDatas() {
        List<?> originDatas = param.getOriginDatas();
        ...
        if (!CollectionUtils.isEmpty(originDatas)) {
            ...
            int seqNum = 1;
            for (Object originData : originDatas) {
                List<String> data = new ArrayList<>(Collections.nCopies(rowCol.getCol(), ""));
                if (null != originData) {
                    for (Field field : fieldSortMap.keySet()) {
                        Header header = field.getAnnotation(Header.class);
                        String name = header.name();
                        if ("序号".equals(name)) {
                            data.set(fieldSortMap.get(field), String.valueOf(seqNum++));
                        } else {
                            Object value = getFieldValue(field, originData);
                            if (null != value) {
                                Class<? extends ValueConverter<?>> covert = header.covert();
                                String cellData = "";
                                if (covert != null) {
                                    ValueConverter serializer = tempCache.getSerializer(covert);
                                    if (serializer != null) {
                                        cellData = serializer.convert(value);
                                    } else {
                                ...
                                data.set(fieldSortMap.get(field), cellData);
                            }
                        }
                    }
                }
    ...
    private Method getMethod(Field field, Object obj) throws NoSuchMethodException {
        Header header = field.getAnnotation(Header.class);
        return obj.getClass()
                .getMethod(StringUtils.isEmpty(header.method()) ? getDefaultMethodName(field) : header.method());
    }
    ...
    private String getDefaultMethodName(Field field) {
        ...
        String result = StringUtils.join(pre, name.substring(0, 1).toUpperCase());
        if (name.length() > 1) {
            result = StringUtils.join(result, name.substring(1));
        }
        return result;
    }
    ...
    private class TempCache {
        ...
        /** cache value converters */
        HashMap<Class<? extends ValueConverter>, ValueConverter> serializerMap = new HashMap<>();

        public ValueConverter getSerializer(Class<? extends ValueConverter> serializerClass) {
            if (serializerClass == null) {
                return null;
            }
            ValueConverter serializer;
            if (serializerMap.containsKey(serializerClass)) {
                serializer = serializerMap.get(serializerClass);
            } else {
